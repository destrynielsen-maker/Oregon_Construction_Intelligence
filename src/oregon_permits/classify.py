...
def classify_permit(p: Permit) -> Permit:
    ...
    authoritative_family = raw.get("source_building_family_authoritative") is True
    text = " ".join([p.permit_type or "", work, use, desc])
    evidence = " ".join([work, use, desc])

    if raw.get("requires_issued_validation") is True and raw.get("source_issued_validated") is not True:
        return _other(p)
    ...
    is_new = raw.get("source_new_construction_subtype") is True or strong_new or (generic_new and not non_new_signal)
    if not is_new:
        return _other(p)
    ...
    if (authoritative_construction or authoritative_family) and report_kind == "residential":
        if explicit_multi:
    ...
    elif (authoritative_construction or authoritative_family) and report_kind == "commercial":
