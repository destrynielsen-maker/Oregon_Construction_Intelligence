...
EXCLUDE_USE = re.compile(r"\b(garage/carport|accessory dwelling|adu\b|shed\b)\b", re.I)
...
def classify_permit(p: Permit) -> Permit:
    ...
    report_kind = str(raw.get("report_kind") or "")
    text = " ".join([p.permit_type or "", work, use, desc])

    if DERIVATIVE.search(p.permit_number):
        return _other(p)

    is_new = bool(re.search(r"\bnew construction\b|construct(?:ion)?\s+new|new\s+(?:single[- ]family|dwelling|building|home|residence)", text, re.I))
    if not is_new or EXCLUDE_USE.search(use):
        return _other(p)

    units = int(p.units or 0)
    if MULTI.search(use) or MULTI.search(desc):
        p.classification = "MULTIFAMILY"
    elif SINGLE.search(use) or SINGLE.search(desc):
        p.classification = "SINGLE_FAMILY"
    elif report_kind.lower() == "commercial" or COMMERCIAL.search(use):
